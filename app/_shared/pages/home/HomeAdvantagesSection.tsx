  ...
  return (
    ...
    >
      <div className="rounded-3xl border border-[#049e6b]/20 bg-white/95 p-8 shadow-sm backdrop-blur-sm md:p-12">
        ...
        <div className="mt-10 grid grid-cols-2 gap-4 xl:mt-12 xl:grid-cols-4">
          {items.map((item, index) => {
            const Icon = item.icon;
            return (
              <article
                key={`${item.title}-${index}`}
                className="group relative flex h-full flex-col items-center gap-3 overflow-hidden rounded-xl border border-[#049e6b]/15 bg-white/90 p-4 shadow-[0_16px_40px_-24px_rgba(4,158,107,0.45)] transition-all duration-200 hover:-translate-y-1 hover:border-[#049e6b]/40 hover:shadow-[0_20px_45px_-20px_rgba(4,158,107,0.55)] md:items-start md:gap-4 md:rounded-2xl md:p-6"
              >
