...
export async function submitOneTimeOrder(
  ...
  try {
    ...
    const selectionCookie = cookieStore.get(CART_SELECTED_MERCHANDISE_COOKIE)?.value;
    ...
    if (webhook) {
      const blocks = [
        { type: "header", text: { type: "plain_text", text: `新的一次性订单 ${orderId}` } },
        { type: "section", fields: [
          { type: "mrkdwn", text: `*姓名*: ${payload.name}` },
          { type: "mrkdwn", text: `*电话*: ${payload.phone || "-"}` },
          { type: "mrkdwn", text: `*微信*: ${payload.wechat || "-"}` },
        ]},
        { type: "section", text: { type: "mrkdwn", text: `*地址*: ${[payload.country, payload.province, payload.city, payload.district, payload.address1].filter(Boolean).join(" ")}${payload.postalCode ? `（${payload.postalCode}）` : ""}` } },
        ...(payload.note ? [{ type: "section", text: { type: "mrkdwn", text: `*备注*: ${payload.note}` } }] : []),
        { type: "divider" },
        { type: "section", text: { type: "mrkdwn", text: checkoutCart.lines.map((l) => `• ${l.merchandise.product.title} × ${l.quantity} —— ${l.cost.totalAmount.amount} ${l.cost.totalAmount.currencyCode}`).join("\n") } },
        { type: "context", elements: [ { type: "mrkdwn", text: `应付总计：${checkoutCart.cost.totalAmount.amount} ${checkoutCart.cost.totalAmount.currencyCode}` } ] },
      ];
      try {
        await fetch(webhook, {
          method: "POST",
    ...
    try {
      if (checkoutCart.lines.length > 0) {
        await removeFromCart(checkoutCart.lines.map((l) => l.id || l.merchandise.id));
      }
      const cookieOptions = await getCartCookieOptions();
      // 清除“结算选择的行”
      const ck = await cookies();
      ck.set({ name: CART_SELECTED_MERCHANDISE_COOKIE, value: "", maxAge: 0, path: "/", httpOnly: false, sameSite: "lax" });
    } catch {
      // 忽略清空失败
    }
